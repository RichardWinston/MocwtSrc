...
C   FIND TOTAL VALUE OF TFUNCTIONS FOR INACTIVE CELLS
      ...
      DO 45 N=NNZER+1,8
        SUMI=SUMI+TFVAL(N)
45    CONTINUE
cgzh  3/25/08 commented out debugging print statements
c
c      if (abs(suma+sumi-1).gt.precno) then
c          print *,'sum tfs ne 1',
c     *                           sumi-1,pc,pr,pl,xhat,yhat,zhat
c      endif
        val=0.D0
      do 4 m=1,nnzer
4      val=val+sumi*temp(m)
c      if (abs(val-sumi).gt.precno) 
c     *          print *,'distr-actual inact',val-sumi
c
c**********************************************************************
