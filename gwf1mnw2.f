...
c  34 is the number of records in MNWNOD
      ispmnwn  = 34 * NODTOT   
      isumz = isumz+ispmnwn
...
c     ------------------------------------------------------------------
     ...
     & small,Qtemp,Hlift,LIFTq0,LIFTqdes,LIFTn,Qn,HWtol
      ...
      dimension mnw2(NMNWVL,mnwmax),MNWNOD(34,NODTOT),MNWINT(11,NODTOT)
...
c
c------------------------------------------------------------------
c     The 32+ rows of the MNW2 array store:
c      Row #  = Description
c------------------------------------------------------------------
c         1   = IACTIV (0: inactive; 1: active)
c         2   = NNODES (absolute value represents number of nodes in this well. Negative values are a flag that well intervals were defined.) 
...
c        18   = qnet
c        19   = PPFLAG
c        20   = Check if potential flow rate is below cutoff. If > 0, limit has been met
c        21   = horizontal well flag
c        22   = PUMPCAP
c        23   = Hlift
c        24   = CapMult
c        25   = apply pump capacity constraints
c        26   = lastQ
c        27   = CapFlag2
c        28   = HWtol
c        29   = qactCap, mnw2(29,iw)=lastQ*0.75d0 or mnw2(29,iw)=lastQ*1.25d0
c        30   = Qpot
c        31   = concw, Well-Conc
c        32   = active transport well flag
c        33+  = Auxiliary values
c------------------------------------------------------------------
! MNWNOD
...
!      10         PNode
!      11         CWCNode
!      12         firstint: first interval intersecting this node. --- INTNUM+IINT-1, 
!                   INTNUM is location in int list of 1st interval for this well
!      13         lastint: last interval intersecting this node --- 
!                   INTNUM+IINT-1, IINT is loop counter for intervals
!      14         cond, conductance of each node
!      15         hwell, hlim, bottom, hdry, "Qseep flag", 
!                   "MNWNOD(15) stores value of bottom to calculate seepage"
!      16         Txx, Transmissivity in the X direction
!      17         Tyy, Transmissivity in the Y direction
!      18         dhp, use saved partial penetration effect if steady state 
!                   and past 1st iter
!      19         PP = partial penetration
!      20         ZPD, topscreen
!      21         bottomscreen
!      22         Unused?
!      23         0, top1-z1, top2-z, or top2-z2, "in first node, use vertical section 
!                   up to top or WT for segment 1", lxf, lyf, lzf, length of well in cell?
!      24         0 or top1-bot1, if this is a vertical segment, define lengths with elevations, 
!                   skip other calc, lxf, lyf, lzf
!      25         distqnce between node exit intersection points
!      26         z1, save z coords as we don't want z screen elevations to change for WT cases
!      27         MNWNOD(27,m) is flow between nodes (saved at upper face) in borehole
!      28         omega
!      29         theta
!      30         cell to well conductance, cond1
!      31         CWC of second segment of node, cond2
!      32         cnode, concentration of node
!      33         pump node flag
!      34         output unit for ptob
c------------------------------------------------------------------
c
...
C     ------------------------------------------------------------------
     ...
     & MNWINT,QCUT,qnet
      dimension mnw2(NMNWVL,MNWMAX),MNWNOD(34,NODTOT),MNWINT(11,INTTOT)
...
c     ------------------------------------------------------------------
     ...
     & buff(NCOL,NROW,NLAY),MNWNOD(34,NODTOT),hnew(NCOL,NROW,NLAY),
     & vbvl(4,msum)
      dimension WELLID(mnwmax)
      COMMON /MNW2COM/MNWAUX(5)
      CHARACTER*16 MNWAUX
...
C     ------------------------------------------------------------------
     ...
     & hnew(NCOL,NROW,NLAY),MNWNOD(34,NODTOT),mnw2(NMNWVL,MNWMAX)
...
C     ------------------------------------------------------------------
     ...
     & MNWNOD(34,NODTOT),mnw2(NMNWVL,MNWMAX)
      DIMENSION BOTM(NCOL,NROW,0:NBOTM),
...
c     
     ...
     & MNWNOD(34,NODTOT),IBOUND(NCOL,NROW,NLAY),hnew(NCOL,NROW,NLAY)
      COMMON /DISCOM/LBOTM(999),LAYCBD(999)
C
      verysmall = 1.D-25
      Qsmall = verysmall
...
c
     ...
     & nodepump,ifound,ir,irp,il,ilp,ic,icp,iout,NMNWVL
      ...
      dimension MNWNOD(34,NODTOT),mnw2(NMNWVL,MNWMAX)
