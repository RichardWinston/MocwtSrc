...
C
C     ******************************************************************
!     PTOBLST(*) WellID for observations

!     IPTMNLST
!     IPTMNLST(*,1) Unit number for saving particle observations for MNW
!     IPTMNLST(*,2) Flag indicating whether or not the headers have been printed
!                   to the output file (0 = headers not printed, 1 = headers printed).
C
C INITIALIZE
      IPTOBLST=0
      IPTMNLST=0
...
C
     ...
     *                  MNWsite,WELLID,mxwel2,nwell2,IPTOBMNW,well2,
     *                  PTOBLST,ncol,nrow,nlay,nscol,nsrow,nslay,
     *                  IOUTS,INPTOB, MNW2, MNWMAX, IPTOBLST, MNWNOD,
     *                  NODTOT, NMNWVL)
C
C     ******************************************************************
C
...
C
      ...
      DIMENSION IPTMNLST(NUMPTOB_MNW,2),well2(18,mxwel2)             
      dimension MNWsite(mxwel2),IPTOBMNW(NSCOL,NSROW,NSLAY)
      dimension WELLID(mxwel2)
      dimension mnw2(NMNWVL,MNWMAX)
      DIMENSION IPTOBLST(4,NUMPTOB_MNW)
      DIMENSION MNWNOD(34,NODTOT)
      INTEGER IPTOBMNW2 ! Unit number for exporting particle observations for MNW2 wells.
      INTEGER INODE, firstnode, lastnode
C
      COMMON /SUBGRD/
     *  ISCOL1,ISCOL2,ISROW1,ISROW2,ISLAY1,ISLAY2,ISUBGD
C
...
!              IPTOBMNW(JS,IS,KS)=IPTMNLST(IOB,1)
              IPTOBMNW2 = IPTMNLST(IOB,1);
            else
              IPTOBMNW2 = -1
            endif
            ! Store the unit number for ptob for this well (IPTOBMNW2) in MNWNOD(34) for all nodes in this well.
            firstnode = MNW2(4,m)
            lastnode=MNW2(4,m)+ABS(MNW2(2,m))-1
            do INODE=firstnode,lastnode
              MNWNOD(34,INODE) = IPTOBMNW2
            enddo
...
C  CYCLE THROUGH UNIT NUMBERS, LOOK FOR MATCH IN PRECEEDING OBS
           IF(IPTE.LT.IPTOB) THEN
C  IF MATCH FOUND, SET HEADER FLAG TO TRUE
             IF(IPTOBUN.EQ.IPTMNLST(IPTE,1)) then
               IHEADER=1
               cycle
             ENDIF
           END IF
...
C     *************************************************************
     ...
     *        MNWMAX, MNW2, MNWNOD, NODTOT, NMNWVL)
C     ***************************************************************
! if MNWFLAG = 0, this subroutine was called after advection, 
! otherwise it was called before advection.
Cgzh MNWFLAG is passed in >0 (=1) when called before advection for complex MNWs
      DOUBLE PRECISION SUMWT,PTWT,WTREM,MSOUT
      DOUBLE PRECISION MNW2,MNWNOD
      CHARACTER*50  LFRMAT
	REAL L
	INTEGER IPTOBMNW2 ! MNW2 output unit number
...
C
     ...
     &             PCONC(IP),SNKFLO(JS,IS,KS),0.0,'     NA    ',
     &             '     NA    '
	        END IF
	       END IF
   9     CONTINUE
...
C  DO NUMPTOB_MNW LOCATIONS WITHIN PARTICLE LOOP
      IF(NUMPTOB_MNW.GT.0) THEN
           DO 19 IP=1,NP
             ...
             DO IWELL=1,MNWMAX
               ! check if well is active.
               IF (MNW2(1,IWELL) .gt. 0) THEN
                 firstnode = MNW2(4,IWELL)
                 ! check if this well is used for particle observations
                 if (MNWNOD(34,firstnode).gt.0)then
                   lastnode=MNW2(4,IWELL)+ABS(MNW2(2,IWELL))-1
                   do INODE=firstnode,lastnode
                     if ((K.eq.MNWNOD(1,INODE))
     &                 .and.(I.eq.MNWNOD(2, INODE))
     &                 .and.(J.eq.MNWNOD(3, INODE))) then
                          IPTOBMNW2 = MNWNOD(34,INODE)
                          cycle
                     endif
                   enddo
                   if (IPTOBMNW2.gt.0) cycle
                 endif
               ENDIF
  30         enddo              
C If printing out this well, IPTOBMNW will have a non-zero unit #
!             IF(IPTOBMNW(JS,IS,KS).GT.0) THEN
             IF(IPTOBMNW2.GT.0) THEN
...
Cljk avoid divide by zero if no particles
cljk              IF(SNKFLO(JS,IS,KS).LT.0.0.AND.PTWTON.GT.0) THEN
! RBW                 IF((SNKFLO(JS,IS,KS).LT.0.0.AND.PTWTON.GT.0).and.
                 IF((PTWTON.GT.0).and.
     +             (sumwt(js,is,ks).gt.0)) THEN
...
C
     ...
     &                  ' after_advection'
                   ELSE
                     WRITE(IPTOBUN,155) J,I,K,SUMTCH,'     NA    ',
     &                  PCONC(IP), SNKFLO(JS,IS,KS),0.0,'     NA    ',
     &                  '     NA    ', ' after_advection'
                   END IF
 	           END IF
Cgzh MNWFLAG is passed in >0 (=1) when called before advection for complex MNWs
               ELSE
                 IF(SNKMNW(JS,IS,KS).LT.0.0) THEN
                   IF(PTWTON.EQ.1) THEN
                     WRITE(IPTOBUN,145) J,I,K,SUMTCH,PTWT(IP),PCONC(IP),
     &                 0.0,SNKMNW(JS,IS,KS),WTREM,MSOUT,
     &                 'before_advection'
                   ELSE
                     WRITE(IPTOBUN,155) J,I,K,SUMTCH,'     NA    ',
     &                 PCONC(IP), 0.0,SNKMNW(JS,IS,KS),'     NA    ',
     &                 '     NA    ', 'before_advection'
                   END IF
                 END IF
	         END IF
