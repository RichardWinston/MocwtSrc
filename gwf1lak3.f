Cdep  Added a term SURFDPTH 3/16/2009 so as to compute a transitionn of 
...
Cdep  3/15/2009 added SURFDEPTH to end of terms in allocate subroutine 
     ...
     * LCWITDW,LCGWRAT,SURFDEPTH)
C
C----- USGS VERSION 3; MAY 2006 GWF1LAK3ALP  DEP; revised 3/15/2009
C
C      ******************************************************************
...
Cdep  initialize number of iterations and closure criteria to zero.
      ...
      NSSITR = 0
      SSCNCR = 0.0
Cdep  3/15/2009 initialize SURFDEPTH to 1.0
      SURFDEPTH= 1.0
C
...
Cdep     LAKE Package.
      ...
      ELSE IF(THETA.LT.0.0)THEN
        THETA=ABS(THETA)
Cdep 3/15/2009  Added new input variable named SURFDEPTH
        IF(IFREFM.EQ.0) THEN
          READ(IN,'(F10.2,I10,2F10.2)') DUM,NSSITR,SSCNCR,SURFDEPTH
        ELSE
          READ(IN,*) DUM,NSSITR,SSCNCR,SURFDEPTH
        END IF
      END IF
...
1     FORMAT(/1X,'LAK3 -- LAKE PACKAGE, VERSION 3.02, 3/15/2009',
...
C1A1----READ INITIAL CONDITIONS FOR ALL LAKES (ONLY READ ONCE)
 ...
 30         CONTINUE
         ELSE
            WRITE (IOUTS,21) NSOL
            WRITE (LFRMAT,23) NSOL !dep 3/15/2009 Note LFRMAT format is not set
            DO 35 LM=1,NLAKES
               IF (IFREFM.EQ.0) THEN
...
Cdep  ADDED SURFDEPTH to end of call statement 3/15/2009.
...
C
C----- USGS VERSION 3; MAY2006 GWF1LAK3FM  DEP; revised 3/15/2009
C
Cdep  MODIFIED SUBROUTINE TO ITERATIVELY SOLVE FOR LAKE STAGE EVEN
C     DURING TRANSIENT STRESS PERIODS. MADE REVISIONS 3/15/2009
C     ******************************************************************
...
Cdep  Added DIMENSION statement for inflow limited lakes
     ...
     1           FLWITER(NLAKES),GWRATELIM(NLAKES)
      DOUBLE PRECISION DLSTG, STGITERDBL
...
Cdep  Compute flow into lake from surface sources
      DO LAKE = 1,NLAKES
...
Cdep       3/15/2009 Corrected nonconvergence problem for dry lake  
Cdep       cells when water table near top of lakebed 
Cdep       Changed to statement 515 
Cdep       SET TOP AND BOTTOM ELEVATIONS OF LAKEBED WITHIN A CELL      
  508    BOTLKUP = BOTLK + 0.5*SURFDPTH
         BOTLKDN = BOTLK - 0.5*SURFDPTH 
         CONDMX = CONDUC
         IF(STGON.LE.BOTLK.AND.H.LE.BOTLKDN) GO TO 547
Cdep    revised if statements and changed THETA to THET1
         IF(STGNEW(LAKE)-BOTLK.GT.CLOSEZERO) THEN
           FLOBO2=CONDUC*(STGNEW(LAKE)-H)
           FLOBO3=CONDUC*(STGNEW(LAKE)+DLSTG-H)
           IF(STGOLD(LAKE)-BOTLK.GT.CLOSEZERO) THEN
             FLOBO1=CONDUC*(STGOLD(LAKE)-H)
           ELSE
             FLOBO1=CONDUC*(BOTLK-H)
           END IF
             FLOBOT = THET1*FLOBO2 + (1.0D0-THET1)*FLOBO1
             FLOBO3 = THET1*FLOBO3 + (1.0D0-THET1)*FLOBO1
Cdep    made a check for when aquifer head greater than 0.5*surfdepth
         ELSE 
           IF (H-BOTLKUP.GT.CLOSEZERO)THEN
             FLOBO2=CONDUC*(BOTLK-H)
             FLOBO3=CONDUC*(BOTLK+DLSTG-H)
               IF(STGOLD(LAKE)-BOTLK.GT.CLOSEZERO) THEN
                 FLOBO1=CONDUC*(STGOLD(LAKE)-H)
               ELSE
                 FLOBO1=CONDUC*(BOTLK-H)
               ...
               FLOBOT = THET1*FLOBO2 + (1.0D0-THET1)*FLOBO1
               FLOBO3 = THET1*FLOBO3 + (1.0D0-THET1)*FLOBO1
           ELSE IF (H-BOTLKDN.GT.-CLOSEZERO)THEN
               CONDUC=CONDMX-(CONDMX/SURFDPTH)*
     +                  ((BOTLK+0.5*SURFDPTH)-H) 
               FLOBO2=CONDMX*(BOTLK-H)
               FLOB03=CONDMX*(BOTLK+DLSTG-H)
               IF (STGOLD(LAKE)-BOTLK.GT.-CLOSEZERO) THEN
                 FLOBO1=CONDMX*(STGOLD(LAKE)-H)
               ELSE
                 FLOBO1=CONDMX*(BOTLK-H)
               END IF
               FLOBOT = THET1*FLOBO2 + (1.0D0-THET1)*FLOBO1
               FLOBO3 = THET1*FLOBO3 + (1.0D0-THET1)*FLOBO1
           END IF
...
Cdep    changed BOTLK to BOTLKDN  3/15/2009
           IF (STGON.LE.BOTLK.AND.ABS(FLOBOT).LT.CLOSEZERO) GO TO 600
           IF (H.LE.BOTLKDN) GO TO 870
           IF (H.GT.BOTLKDN) GO TO 850
...
Cdep    3/15/2009 Changed BOTLK to BOTLKDN  
...
Cdep          SURFA(LAKE)=SURFA(LAKE)+ AREA
              STGITERDBL = STGITER(LAKE)
              SURFA(LAKE)=FINTERP(STGITERDBL,LAKE,NLAKES,AREATABLE,
     +              DEPTHTABLE)
Cdep       ENDIF
900      CONTINUE
C
...
Cdep  Test for transient or steady-state
     ...
     2                DELT/FINTERP(STGITERDBL,LAKE,NLAKES,AREATABLE,
     3                DEPTHTABLE)-(STGITERDBL-STGOLD(LAKE))
           END IF
     ...
     2                DELT/FINTERP(STGITERDBL+DLSTG,LAKE,NLAKES,
     3                AREATABLE,DEPTHTABLE)-(STGITERDBL+
     4                DLSTG-STGOLD(LAKE))
         ELSE
           IF(STGNEW(LAKE)-BOTTMS(LAKE).LT.CLOSEZERO) THEN
             RESID1 = FLWITER(LAKE)*DELT/AREATABLE(1,LAKE)
...
C
Cdep  3/15/2009  added SURFDEPTH to end of terms
...
Cdep  added double precision variables THET1, DLKSTAGE, SLKOTFLW
      DOUBLE PRECISION THET1, DLKSTAGE, SLKOTFLW
Cdep  3/15/2009 added double precision variables SURFDPTH,CONDMX
Cdep  3/15/2009 added double precision variables BOTLKUP, BOTLKDN
      DOUBLE PRECISION SURFDPTH,CONDMX,BOTLKUP,BOTLKDN
      DIMENSION CNDFCT(MXLKND),ILAKE(5,MXLKND),HNEW(NCOL,NROW,NLAY),
...
C       SEEPAGE INTO OR OUT OF A LAKE WALL NODE, WHEN ITYPE=1 OR 2.
  ...
  506       FORMAT(1X,'ERROR - NO AQUIFER UNDER LAKE CELL ',4I5)
            CALL USTOP(' ')
  507       H = BOTLK
Cdep     changed 3/15/2009 to stop numerical oscillations across
Cdep      lakebed when lake stage less than lake bottom
Cdep  508      IF(STGNEW(LAKE).LE.BOTLK.AND.H.LE.BOTLK) GO TO 549
Cdep    revised if statements and changed THETA to THET1
  508    BOTLKUP = BOTLK + 0.5*SURFDPTH
         BOTLKDN = BOTLK - 0.5*SURFDPTH
         CONDMX = CONDUC
         IF(STGNEW(LAKE).LE.BOTLK.AND.H.LE.BOTLKDN) GO TO 549
           IF(STGNEW(LAKE)-BOTLK.GT.CLOSEZERO) THEN
             FLOBO2=CONDUC*(STGNEW(LAKE)-H)
             IF(STGOLD(LAKE)-BOTLK.GT.CLOSEZERO) THEN
               FLOBO1=CONDUC*(STGOLD(LAKE)-H)
...
C
C9-------COMPUTE ACCUMULATIVE SEEPAGE THROUGH LAKEBED SEDIMENTS.
Cdep   Seep iS a volumetric rate. Removed DELT from equation
         SEEP(LAKE)=SEEP(LAKE)-FLOBOT
C
C10------COMPUTE ACCUMULATIVE LAKE NODAL AREA, EXCLUDE WALL NODES.
