...
c   cntrib = concentration in the tributary, this is set here because
c   we are at the first reach of a new segment, so the last reach of
c   the last segment COUT(LL) will be the right C for water leaving that seg.
         IF (ISTSG.GT.1) THEN
             IFLG=ISTRM(4,LL)
             DO 15 ISOL=1,NSOL
